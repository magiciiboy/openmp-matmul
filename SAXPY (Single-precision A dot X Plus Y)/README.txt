...
1. saxpy.c : SAXPY without any parallelization.
2. saxpy_both_simple.c : SAXPY with both loops parallelized.
3. saxpy_inner_simple.c : SAXPY with inner loop parallelized.
4. saxpy_outer_simple.c : SAXPY with outer loop parallelized.
...
7. Extras (directory) : Contains SAXPY with some advanced openMP clauses.
8. rapl-read.c
...
>> As said above, you will gather metrics for each thread case individually, for example: if you want the above mentioned metrics for "saxpy_outer_simple.c":
	...
	> Now compile the code using TAU with "-fopenmp" flag:
		- [cXYZ-ABC]$ tau_cc.sh -fopenmp saxpy_outer_simple.c -o out_simple_tau
...
>> Example: to use process pinning, for "saxpy_outer_simple.c":
	...
	> Compile your code just like normal:
		- [cXYZ-ABC]$ icc -O1 saxpy_outer_simple.c -o out_simple
